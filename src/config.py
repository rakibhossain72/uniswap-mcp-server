...
class Config(BaseModel):
    ...
    def load(cls) -> "Config":
        ...
        # Load .env file if present — does nothing when env vars are already set.
        load_dotenv()

        rpc_url = os.environ.get("RPC_URL")
        if not rpc_url:
            raise ValueError(
                "RPC_URL environment variable is required to run the server."
            )
...
# Backward-compatible alias so existing code using `from src.config import settings`
# still works — the actual load is deferred until this attribute is first accessed.
class _LazySettings:
